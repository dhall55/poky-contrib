...
python check_bblayers_conf() {
    bblayers_fn = os.path.join(d.getVar("TOPDIR"), "conf/bblayers.conf")
    ...
    bb.note("Merging bblayers.conf - meta")
    if current_lconf < 4:
        sys.exit()
    elif current_lconf == 4:
        topdir_var = '$' + '{TOPDIR}'
...
def check_sanity(sanity_data):
    ...
    if current_lconf != lconf_version:
        try:
            ...
            messages = messages + "Your bblayers.conf has been updated. Please re-run."
