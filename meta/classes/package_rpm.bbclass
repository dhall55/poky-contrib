...
package_update_index_rpm () {
	basearchs="${PACKAGE_ARCHS}"

	# For a tri-arch, this should really be an array for each arch set..
	# but we'll hack it for the bi-arch
	mlarchs="${MULTILIB_ARCHS}"

	if [ ! -z "${DEPLOY_KEEP_PACKAGES}" ]; then
		return
	fi

	for archvar in basearchs mlarchs; do
		eval archs=\${${archvar}}
		packagedirs=""
		packagedirs_sdk=""
	...
	if [ x${TARGET_OS} = "xlinux" ] || [ x${TARGET_OS} = "xlinux-gnueabi" ] ; then
		if [ ! -z "${package_lingusa}" ]; then
			for pkg in ${package_lingusa}; do
				...
				archvar=basearchs
				ml_pkg=$(echo ${pkg} | sed "s,${MULTILIB_ID}\(.*\),\1,")
				if [ "${ml_pkg}" != "${pkg}" ]; then
					archvar=mlarchs
				fi
