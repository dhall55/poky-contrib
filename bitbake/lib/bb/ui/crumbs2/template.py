...
class HobTemplateFile(ConfigFile):
    ...
    def getVar(self, var):
        if var in self.dictionary: 
            vallist = self.dictionary[var].split()
        else:
            return None
        if len(vallist) > 1:
            return vallist
...
class TemplateMgr(gobject.GObject):
    ...
    def open(self, path):
        self.template_hob = HobTemplateFile("%s%s" % (path, "_template.hob"))
        self.bblayers_conf = ConfigFile("%s%s" % (path, "_bblayers.conf"))
        self.local_conf = ConfigFile("%s%s" % (path, "_local.conf"))
        self.image_bb = RecipeFile("%s%s" % (path, "_image.bb"))
